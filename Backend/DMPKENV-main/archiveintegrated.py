...
DB_CONFIG = {
    ...
    'password': os.getenv('DB_PASSWORD', 'rgjPtKwPcPlbBrZJOeAFSeQgAjULowPu'), # Wajib ada di environment
...
def db_summary():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) AS total FROM emotion_track WHERE YEARWEEK(timestamp,1) = YEARWEEK(CURDATE(),1)")
        ...
        cursor.execute("SELECT COUNT(*) AS total FROM emotion_track WHERE YEARWEEK(timestamp,1) = YEARWEEK(CURDATE() - INTERVAL 7 DAY,1)")
        ...
        cursor.execute("SELECT emotion, COUNT(*) AS count FROM emotion_track WHERE YEARWEEK(timestamp,1) = YEARWEEK(CURDATE(),1) GROUP BY emotion")
        this_week_counts_raw = cursor.fetchall()
        this_week = {r['emotion']: (r['count'] / total_this_week) * 100 for r in this_week_counts_raw}


        cursor.execute("SELECT emotion, COUNT(*) AS count FROM emotion_track WHERE YEARWEEK(timestamp,1) = YEARWEEK(CURDATE() - INTERVAL 7 DAY,1) GROUP BY emotion")
